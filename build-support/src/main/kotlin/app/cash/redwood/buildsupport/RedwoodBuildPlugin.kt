...
private const val REDWOOD_VERSION = "0.20.0-composelive-0.11-SNAPSHOT"
...
class RedwoodBuildPlugin : Plugin<Project> {
  ...
  private fun Project.configureCommonKotlin() {
    ...
    // Package the bridge keep-names list (generated by the zipline bridge plugin during
    // compileKotlinJvm) into the JVM jar, so consumers can aggregate the `-Xir-keep` field
    // names from published artifacts. The path is namespaced per module so that Android
    // packaging doesn't see duplicate resources. Modules without the bridge plugin have no file.
    val projectName = name
    tasks.withType(org.gradle.api.tasks.bundling.Jar::class.java)
      .matching { it.name == "jvmJar" }
      .configureEach {
        it.from(
          layout.buildDirectory.dir("generated/bridge/c"),
          org.gradle.api.Action { spec: org.gradle.api.file.CopySpec ->
            spec.include("bridge-keep-names.txt")
            spec.rename("bridge-keep-names.txt", "META-INF/redwood-bridge-keep-names/$projectName.txt")
          },
        )
      }
  }
